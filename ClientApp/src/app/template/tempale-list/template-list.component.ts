...
import { Option } from '../template/layout-models';
...
import { CodezenUiRendererComponent, RendererType } from 'codezen-ui-layout';
...
export class TemplateListComponent implements OnInit, OnChanges, OnDestroy {
  ...
  public rendererType = RendererType.ADD;
  ...
  public onSearch(): void {
    ...
    if (this.layoutRenderer) {
      this.form = this.layoutRenderer.form;
    }
    for (const [key, value] of Object.entries(this.form?.value ?? {}) as [string, any]) {
      const guidData = this.layoutRenderer?.layout.find(o => o.fieldName === key)?.dataSource?.find(o => o.id === value);
  ...
  private loadDropDownData(): void {
    const dropDownFields = this.getAllDropDownFields(this.filterFields);
